...
@section('content')
        ...
        <div
            ...
            <div
                ...
                <div class="flex items-center space-x-3.5">
                    ...
                    <div>
                        <h4 class="font-black text-slate-900 uppercase tracking-tight text-sm">Manajemen {{ $judul }}</h4>
                        <p class="text-[11px] text-slate-500 font-medium mt-0.5">Konfigurasi dokumen elektronik berkaitan program dan kegiatan Tahun 2022 – 2024.</p>
                    </div>
                </div>
                <span
            ...
            @php
                $currentSlug = str_replace('ppid-', '', $item->slug);
            @endphp

            <form action="{{ route('admin.ppid.update', $currentSlug) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                    ...
                    <div class="rounded-2xl overflow-hidden border border-slate-200 shadow-2xs bg-white">
                        <div id="editor-main">{!! old('konten', $item->content_data ?? '') !!}</div>
                    </div>
                ...
                <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-slate-50/40 p-5 rounded-2xl border border-slate-200/50">
                        <div class="lg:col-span-4 w-full aspect-video bg-white rounded-xl border border-slate-200 flex items-center justify-center overflow-hidden shadow-2xs shrink-0 group relative">
                            @if (isset($item->primary_image_path) && $item->primary_image_path)
                                <img id="preview-gambar" src="{{ Storage::url($item->primary_image_path) }}" class="w-full h-full object-contain">
                            @else
                                <img id="preview-gambar" src="https://via.placeholder.com/400x250?text=Format+Infografis" class="w-full h-full object-contain opacity-20">
                            @endif
                        </div>
                ...
                <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-slate-50/40 p-5 rounded-2xl border border-slate-200/50">
                        <div class="lg:col-span-4 w-full aspect-video bg-white rounded-xl border border-slate-200 flex items-center justify-center overflow-hidden shadow-2xs shrink-0 group relative">
                            @if (isset($item->secondary_image_path) && $item->secondary_image_path)
                                <img id="preview-gambar-2" src="{{ Storage::url($item->secondary_image_path) }}" class="w-full h-full object-contain">
                            @else
                                <img id="preview-gambar-2" src="https://via.placeholder.com/400x250?text=Format+Infografis" class="w-full h-full object-contain opacity-20">
                            @endif
                        </div>
                ...
                <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-3">
                            <div class="h-7 w-7 rounded-lg bg-red-50 border border-red-200 text-red-600 flex items-center justify-center font-black text-xs shadow-2xs">
                                ●
                            </div>
                            <label class="block text-xs font-black text-slate-900 uppercase tracking-[0.15em]">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center bg-slate-50/40 p-5 rounded-2xl border border-slate-200/50">
                        <div class="lg:col-span-1 h-12 w-12 bg-white border border-slate-200 rounded-xl flex items-center justify-center mx-auto shadow-2xs">
                        ...
                        <div class="lg:col-span-11 w-full space-y-3">
                            ...
                            @if (isset($item->primary_document_path) && $item->primary_document_path)
                                <div class="bg-white border border-emerald-200 p-3 rounded-xl flex items-center space-x-2.5 shadow-3xs">
                                    ...
                                    <a href="{{ Storage::url($item->primary_document_path) }}" target="_blank"
                                        ...
                                        Berkas Aktif: Lihat / Unduh Dokumen Lampiran
                                    </a>
                                </div>
                            @endif
                        </div>
            ...
            <form id="form-hapus-komponen" action="{{ route('admin.ppid.update', $currentSlug) }}" method="POST" class="hidden">
                @csrf
    ...
    <script>
        var quillHero = new Quill('#editor-hero', {
            theme: 'snow',
            placeholder: 'Ketik deskripsi singkat untuk banner hero...',
            modules: { toolbar: [['bold', 'italic', 'underline'], [{ 'list': 'ordered'}, { 'list': 'bullet' }], ['clean']] }
        });
        quillHero.on('text-change', function() { document.getElementById('hidden-hero').value = quillHero.root.innerHTML; });

        var quillMain = new Quill('#editor-main', {
            theme: 'snow',
            placeholder: 'Ketik isi konten utama di sini...',
            modules: { toolbar: [['bold', 'italic', 'underline', 'blockquote'], [{ 'list': 'ordered' }, { 'list': 'bullet' }], ['link'], ['clean']] }
        });
        quillMain.on('text-change', function() { document.getElementById('hidden-konten').value = quillMain.root.innerHTML; });
        ...
        function confirmDeleteSection(type) {
            let namaKomponen = type === 'text' ? 'NARASI TEKS' : (type === 'image' ? 'GAMBAR' : 'DOKUMEN PDF');
            if (confirm("Apakah Anda yakin ingin menghapus komponen " + namaKomponen + "?")) {
                if (confirm("⚠️ PERINGATAN KEDUA:\nTindakan ini bersifat PERMANEN.\n\nApakah Anda benar-benar serius?")) {

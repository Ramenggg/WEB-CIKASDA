...
@section('title', 'Kelola SK GUB Bangunan Gedung 2025')
...
@section('content')
        ...
        <div class="w-full bg-white/90 backdrop-blur-md rounded-3xl shadow-[0_4px_30px_rgba(15,23,42,0.04)] border border-slate-200/80 overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-100 bg-linear-to-r from-slate-50 via-white to-slate-50 flex justify-between items-center">
                <div class="flex items-center space-x-3.5">
                    ...
                    <div>
                        <h4 class="font-black text-slate-900 uppercase tracking-tight text-sm">Manajemen Halaman SK GUB</h4>
                        <p class="text-[11px] text-slate-500 font-medium mt-0.5">Konfigurasi naskah hero dan file SK Gubernur yang dapat diunduh publik.</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.informasi.update', 'sk-gub') }}" method="POST" enctype="multipart/form-data" class="divide-y divide-slate-100">
                @csrf
                ...
                <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <label class="block text-xs font-black text-slate-900 uppercase tracking-[0.15em]">Berkas SK Gubernur 2025</label>
                                <span class="text-[10px] text-slate-400 font-semibold">Dokumen resmi SK Gubernur Bangunan Gedung</span>
                            </div>
                        </div>
                        ...
                        <div class="lg:col-span-11 w-full space-y-3">
                            ...
                            @if (isset($item->primary_document_path) && $item->primary_document_path)
                                <div class="bg-white border border-emerald-200 p-3 rounded-xl flex items-center space-x-2.5 shadow-3xs text-emerald-700 font-bold text-xs underline decoration-emerald-300 underline-offset-4">
                                    ...
                                    <a href="{{ Storage::url($item->primary_document_path) }}" target="_blank">Lihat Berkas PDF Aktif</a>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <form id="form-hapus-komponen" action="{{ route('admin.informasi.update', 'sk-gub') }}" method="POST" class="hidden">
                @csrf
    ...
    <script>
        ...
        function confirmDeleteSection(type) {
            let check1 = confirm("Apakah Anda yakin ingin menghapus file SK ini?");
            if (check1) {

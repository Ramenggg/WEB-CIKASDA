...
@section('title', 'Kelola Perjanjian Kerja Sama (MoU)')
...
@section('content')
        ...
        <div class="w-full bg-white/90 backdrop-blur-md rounded-3xl shadow-[0_4px_30px_rgba(15,23,42,0.04)] border border-slate-200/80 overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-100 bg-linear-to-r from-slate-50 via-white to-slate-50 flex justify-between items-center">
                <div class="flex items-center space-x-3.5">
                    ...
                    <div>
                        <h4 class="font-black text-slate-900 uppercase tracking-tight text-sm">Manajemen MoU</h4>
                        <p class="text-[11px] text-slate-500 font-medium mt-0.5">Konfigurasi naskah hero dan file MoU yang dapat diunduh publik.</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.informasi.update', 'mou') }}" method="POST" enctype="multipart/form-data" class="divide-y divide-slate-100">
                @csrf
                ...
                @php
                    $mous = [
                        ['field' => 'primary_document_path', 'input' => 'pdf_file', 'label' => 'MoU Penyediaan SPAM Lintas Wilayah', 'delete' => 'pdf', 'desc' => 'File PDF untuk MoU SPAM Lintas Wilayah'],
                        ['field' => 'secondary_document_path', 'input' => 'pdf_file_2', 'label' => 'MoU Infrastruktur Sanitasi Lingkungan Sehat', 'delete' => 'pdf_2', 'desc' => 'File PDF untuk MoU Infrastruktur Sanitasi']
                    ];
                @endphp
                ...
                @foreach ($mous as $m)
                    <div class="p-8 space-y-4 bg-white hover:bg-slate-50/30 transition-all duration-300">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center space-x-3">
                                <div class="h-7 w-7 rounded-lg bg-blue-50 border border-blue-200 text-blue-600 flex items-center justify-center font-black text-xs shadow-2xs">●</div>
                                <div>
                                    <label class="block text-xs font-black text-slate-900 uppercase tracking-[0.15em]">{{ $m['label'] }}</label>
                                    <span class="text-[10px] text-slate-400 font-semibold">{{ $m['desc'] }}</span>
                                </div>
                            </div>
                            @if (isset($item->{$m['field']}) && $item->{$m['field']})
                                <button type="button" data-target="{{ $m['delete'] }}" onclick="confirmDeleteSection(this.dataset.target)"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center bg-slate-50/40 p-5 rounded-2xl border border-slate-200/50">
                            <div class="lg:col-span-1 h-12 w-12 bg-white border border-slate-200 rounded-xl flex items-center justify-center mx-auto shadow-2xs text-blue-600 font-bold text-xs">PDF</div>
                            <div class="lg:col-span-11 w-full space-y-3">
                                <input type="file" name="{{ $m['input'] }}" accept=".pdf"
                                    class="block w-full text-sm text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-600 text-white hover:file:bg-blue-700 file:transition-all file:cursor-pointer">
                                @if (isset($item->{$m['field']}) && $item->{$m['field']})
                                    <div class="bg-white border border-emerald-200 p-3 rounded-xl flex items-center space-x-2.5 shadow-3xs text-emerald-700 font-bold text-xs underline decoration-emerald-300 underline-offset-4">
                                        ...
                                        <a href="{{ Storage::url($item->{$m['field']}) }}" target="_blank">Lihat Berkas PDF Aktif</a>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            <form id="form-hapus-komponen" action="{{ route('admin.informasi.update', 'mou') }}" method="POST" class="hidden">
                @csrf
    ...
    <script>
        ...
        function confirmDeleteSection(type) {
            let check1 = confirm("Apakah Anda yakin ingin menghapus file MoU ini?");
            if (check1) {

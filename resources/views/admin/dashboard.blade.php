...
@section('content')
    <div class="max-w-7xl mx-auto space-y-8 animate-fade-in">
        ...
        {{-- ==================================================================
         1. WELCOME BANNER (DEEP BLUE GRADIENT WITH WHITE TEXT)
         ================================================================== --}}
        <div
            class="relative overflow-hidden bg-gradient-to-r from-blue-700 via-blue-800 to-indigo-900 rounded-3xl p-6 md:p-8 shadow-md border border-blue-900/10">
            {{-- Decorative pattern overlay --}}
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-blue-500/20 via-transparent to-transparent pointer-events-none"></div>
            ...
            <div class="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-6">
                <div class="text-center lg:text-left space-y-3">
                    <div
                        class="inline-flex items-center space-x-2 bg-white/10 border border-white/20 px-3 py-1 rounded-full">
                        <span class="flex h-1.5 w-1.5 rounded-full bg-cyan-300 animate-pulse"></span>
                        <span class="text-[9px] font-black text-cyan-200 uppercase tracking-widest">Pusat Kendali
                            Portal</span>
                    </div>
                    <h2 class="text-2xl md:text-4xl font-black text-white leading-tight tracking-tight">
                        Halo, <span
                            class="bg-gradient-to-r from-cyan-200 via-blue-200 to-indigo-100 bg-clip-text text-transparent drop-shadow-xs">Admin
                            CIKASDA!</span>
                    </h2>
                    <p class="text-blue-100 max-w-lg font-semibold leading-relaxed text-xs opacity-90">
                    ...
                    <div class="pt-2 flex flex-wrap justify-center lg:justify-start gap-3">
                        <a href="{{ route('admin.berita.tambah') }}"
                            class="px-5 py-2.5 bg-cyan-500 hover:bg-cyan-600 text-slate-950 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 shadow-md shadow-cyan-500/20 active:scale-98 cursor-pointer">
                            Tambah Berita
                        </a>
                ...
                <div
                    class="bg-black/25 backdrop-blur-md border border-white/10 p-5 rounded-2xl w-full lg:w-64 shadow-md">
                    <div class="flex items-center justify-between border-b border-white/10 pb-2.5 mb-3">
                        <p class="text-[9px] font-black text-blue-200 uppercase tracking-widest">Status Sistem</p>
                        <span
                            class="text-emerald-300 font-black tracking-wide text-[9px] bg-emerald-400/25 px-2 py-0.5 border border-emerald-400/35 rounded-md">Stable</span>
                    </div>
                    <div class="space-y-3">
                        <div class="w-full bg-white/10 h-1.5 rounded-full overflow-hidden p-0.5 border border-white/15">
                            <div class="bg-gradient-to-r from-cyan-400 to-blue-400 h-full rounded-full"
                                style="width: 85%"></div>
                        </div>
                        <div
                            class="flex items-center justify-between text-[9px] text-blue-200 font-black uppercase tracking-wider">
                            <span>Waktu server:</span>
                            <span class="text-white">{{ now()->format('H:i') }} WITA</span>
                        </div>
                    </div>
                </div>
            </div>

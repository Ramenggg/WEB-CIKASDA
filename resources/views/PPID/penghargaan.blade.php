...
@section('content')
    <x-profil-hero title="Penghargaan PPID" :item="$item" />
                    ...
                    @else
                        <div class="text-center py-12">
                            <div class="w-20 h-20 mb-6 bg-slate-50 rounded-full border border-slate-100 flex items-center justify-center mx-auto shadow-sm">
                                <svg class="w-10 h-10 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"></path></svg>
                            </div>
                            <h3 class="text-lg font-bold text-slate-700 mb-2">Penghargaan Belum Tersedia</h3>
                            <p class="text-slate-500 text-sm leading-relaxed max-w-md mx-auto">Daftar capaian dan penghargaan PPID akan ditampilkan di sini setelah diperbarui oleh administrator.</p>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-xl overflow-hidden p-6 border border-slate-100 space-y-8">
                        @if ($item->primary_image_path)
                            <div class="relative w-full bg-slate-50/50 flex items-center justify-center overflow-x-auto border border-slate-100 rounded-xl p-4">
                                <img src="{{ Storage::url($item->primary_image_path) }}" alt="Infografis Penghargaan PPID"
                        ...
                        @if ($item->secondary_image_path)
                            <div class="relative w-full bg-slate-50/50 flex items-center justify-center overflow-x-auto border border-slate-100 rounded-xl p-4">
                                <img src="{{ Storage::url($item->secondary_image_path) }}" alt="Infografis Penghargaan PPID 2"
                                    class="w-full h-auto object-contain rounded-2xl shadow-md">
                            </div>
                        @endif
                    </div>
                ...
                @if (isset($item) && $item->primary_document_path)
                    <div class="w-full bg-gradient-to-r from-red-50/50 via-slate-50 to-red-50/20 border border-slate-200 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl">
                        <div class="flex items-center space-x-4 text-center sm:text-left flex-col sm:flex-row gap-4 sm:gap-0">
                            ...
                            <div>
                                <h5 class="text-base font-black text-slate-900 tracking-tight">Dokumen Lampiran Penghargaan PPID</h5>
                                <p class="text-xs text-slate-500 font-semibold mt-0.5">Unduh dokumen PDF sertifikat/penghargaan resmi (Format PDF).</p>
                            </div>
                        </div>

...
@section('content')
    <x-profil-hero title="Permohonan Informasi PPID" :item="$item" />
                    ...
                    @else
                        <div class="text-center py-12">
                            <div class="w-20 h-20 mb-6 bg-slate-50 rounded-full border border-slate-100 flex items-center justify-center mx-auto shadow-sm">
                                <svg class="w-10 h-10 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </div>
                            <h3 class="text-lg font-bold text-slate-700 mb-2">Informasi Belum Tersedia</h3>
                            <p class="text-slate-500 text-sm leading-relaxed max-w-md mx-auto">Prosedur dan formulir permohonan informasi PPID akan ditampilkan di sini setelah diperbarui oleh administrator.</p>
                        </div>
                    @endif

                    <div class="mt-8 flex justify-center">
                        <a href="{{ route('informasi.form-permohonan') }}" 
                           class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-all duration-300 flex items-center space-x-2">
                           <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                           <span>Isi Formulir Permohonan Online</span>
                        </a>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-xl overflow-hidden p-6 border border-slate-100 space-y-8">
                        @if ($item->primary_image_path)
                            <div class="relative w-full bg-slate-50/50 flex items-center justify-center overflow-x-auto border border-slate-100 rounded-xl p-4">
                                <img src="{{ Storage::url($item->primary_image_path) }}" alt="Infografis Permohonan Informasi"
                        ...
                        @if ($item->secondary_image_path)
                            <div class="relative w-full bg-slate-50/50 flex items-center justify-center overflow-x-auto border border-slate-100 rounded-xl p-4">
                                <img src="{{ Storage::url($item->secondary_image_path) }}" alt="Infografis Permohonan Informasi 2"
                                    class="w-full h-auto object-contain rounded-2xl shadow-md">
                            </div>
                        @endif
                    </div>
                ...
                @if (isset($item) && $item->primary_document_path)
                    <div class="w-full bg-gradient-to-r from-red-50/50 via-slate-50 to-red-50/20 border border-slate-200 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl">
                        <div class="flex items-center space-x-4 text-center sm:text-left flex-col sm:flex-row gap-4 sm:gap-0">
                            ...
                            <div>
                                <h5 class="text-base font-black text-slate-900 tracking-tight">Dokumen Lampiran Permohonan</h5>
                                <p class="text-xs text-slate-500 font-semibold mt-0.5">Unduh dokumen PDF terkait permohonan informasi (Format PDF).</p>
                            </div>
                        </div>

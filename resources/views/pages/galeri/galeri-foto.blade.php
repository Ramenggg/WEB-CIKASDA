...
@section('content')
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="w-full flex flex-col items-start text-left">
                    <div
                        ...
                        <span class="text-white font-semibold">Foto</span>
                    </div>

                    <div class="border-l-4 border-blue-500/50 pl-4 md:pl-6 mb-8 mt-4">
                        <h1
                            class="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold font-heading text-white mb-4 tracking-tight relative">
                            Galeri Foto
                        </h1>

                        <div class="text-blue-100 text-sm md:text-base leading-relaxed max-w-2xl">
                            Arsip dokumentasi fisik pembangunan infrastruktur, pengelolaan sumber daya air, dan
                            dokumentasi dinas
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 md:-mt-32 relative z-20">
            <div
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4 bg-slate-50 border border-slate-200/60 rounded-xl px-4 py-3 w-fit">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-wider">Total Kategori</p>
                                <p class="text-sm font-bold text-slate-800 leading-none">{{ $totalAlbum ?? $albums->count() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-slate-800 leading-none">
                                    {{ $totalFoto ?? collect($albums)->sum(fn($a) => $a->fotos->count()) }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex flex-wrap items-center gap-2 mb-8" id="categoryFilterContainer">
                    <button
                        class="kategori-btn active px-4 py-2 rounded-xl text-xs font-bold transition-all bg-blue-600 text-white shadow-md shadow-blue-600/20"
                        data-kategori="umum">Umum</button>
                    @foreach ($kategoriList as $kat)
                        @if (strtolower($kat) !== 'umum')
                        <button
                            class="kategori-btn px-4 py-2 rounded-xl text-xs font-bold transition-all bg-slate-50 text-slate-500 hover:text-slate-800 hover:bg-slate-100 border border-slate-200"
                ...
                <div id="noResultState" class="hidden py-12 text-center w-full col-span-full">
                    <p class="text-slate-500 text-sm">Tidak ditemukan album yang cocok dengan pencarian "<span
                ...
                <div id="albumGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @forelse($albums ?? [] as $album)
                        <div data-fotos="{{ json_encode($album->fotos ?? []) }}" data-judul="{{ $album->judul_album }}"
                            data-deskripsi="{{ $album->deskripsi_album }}"
                            data-kategori="{{ $album->kategori ?: 'Umum' }}"
                            onclick="bukaModalGaleri(JSON.parse(this.dataset.fotos), this.dataset.judul, this.dataset.deskripsi, this.dataset.kategori)"
                            ...
                            <div class="p-6 flex-1 flex flex-col justify-between space-y-4 bg-white relative z-10">
                                <div class="space-y-2">
                                    ...
                                    <div class="inline-block">
                                        <span
                                            class="px-2.5 py-1 rounded-md bg-blue-50 text-blue-600 border border-blue-100 text-[9px] font-black uppercase tracking-widest inline-flex items-center gap-1 shadow-sm">
                                    ...
                                    <p class="text-xs text-slate-400 font-medium leading-relaxed line-clamp-2">
                                        {{ $album->deskripsi_album ?? 'Tidak ada rincian deskripsi tambahan mengenai album foto ini.' }}
                                    </p>
                                </div>
                        ...
                        <div class="col-span-full py-16 text-center space-y-5 max-w-xl mx-auto w-full">
                            <div class="w-32 h-32 mx-auto mb-4 opacity-80 animate-[bounce_3s_infinite]">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                    ...
                                    stroke-linejoin="round" class="w-full h-full text-slate-300">
                            ...
                            <h3
                                class="text-sm md:text-lg font-black text-slate-800 uppercase tracking-widest leading-snug">
    ...
    <script>
        ...
        function bukaModalGaleri(fotos, judul, deskripsi, kategori) {
            if (!fotos || fotos.length === 0) return;

            koleksiFotoAktif = fotos;
            indexAktif = 0;

            document.getElementById('modal-kategori').innerText = kategori ? kategori : 'Umum';
        ...
        function filterAlbums() {
            ...
            cards.forEach(card => {
                ...
                const matchesCategory = activeCategory === 'umum' || kategori === activeCategory;
            ...
            if (visibleCount === 0 && (keyword !== '' || activeCategory !== 'umum')) {

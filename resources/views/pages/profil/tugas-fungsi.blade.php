        ...
        <div class="relative z-10 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-end min-h-[140px]">
            <div class="flex items-center space-x-2 text-[10px] sm:text-xs font-black tracking-widest uppercase">
                <span class="text-slate-400">PROFIL</span> <span class="text-slate-500 font-medium">›</span> <span
                    class="text-blue-400">TUGAS & FUNGSI</span>
            </div>
            <h1 class="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight mt-3 mb-4 drop-shadow-md">Tugas
                dan Fungsi</h1>
            <p class="text-slate-300 text-xs sm:text-sm md:text-base font-medium max-w-4xl leading-relaxed opacity-90">
                Penjabaran tupoksi, wewenang, dan tanggung jawab kerja lini organisasi Dinas Cipta Karya dan Sumber Daya Air
                dalam menyelenggarakan urusan pemerintahan daerah.</p>
        </div>
    </div>
            ...
            <div
                ...
                <div class="px-6 py-4 bg-white border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center space-x-2 shrink-0">
                        ...
                        <span
                            class="text-[11px] text-slate-400 font-bold uppercase tracking-wider pl-4 border-l border-slate-100 ml-2">URAIAN
                            TUPOKSI</span>
                    </div>
                    <div
                ...
                <div class="p-8 sm:p-12 lg:p-16 pt-6 space-y-12">
                    @if (isset($item->konten) && !empty(trim($item->konten)) && $item->konten !== '<p><br></p>')
                        <div class="w-full">
                            <div
                                class="prose prose-slate max-w-none break-words text-slate-700 leading-relaxed font-medium prose-headings:font-black prose-headings:text-slate-900 prose-p:text-base sm:prose-p:text-lg prose-ol:space-y-4 prose-ul:space-y-4 prose-li:text-base sm:prose-li:text-lg">
                                {!! $item->konten !!}
                            </div>
                        </div>
                    @endif
                    ...
                    @if ($item->gambar_path && \Storage::disk('public')->exists($item->gambar_path))
                        <div class="w-full">
                            <div
                                ...
                                <img src="{{ asset('storage/' . $item->gambar_path) }}" alt="Infografis Tugas Fungsi"
                        ...
                        <div class="w-full">
                            <div
                                ...
                                <div
                                    ...
                                    <div>
                                        <h5 class="text-base font-black text-slate-900 tracking-tight">Peraturan Gubernur
                                            Mengenai Tugas Pokok & Fungsi Dinas</h5>
                    ...
                    @if (
                        ...
                        <div class="w-full text-center py-12">
                            <p class="text-slate-400 font-bold">Informasi Tugas & Fungsi belum tersedia.</p>
                        </div>
                    @endif
                </div>
            </div>

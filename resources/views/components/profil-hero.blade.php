            ...
            <div class="border-l-4 border-blue-500/50 pl-4 md:pl-6 mb-8 mt-4">
                ...
                <div class="text-blue-100 text-sm md:text-base leading-relaxed max-w-2xl">
                    @if (!empty(trim(strip_tags($item->hero_description ?? ''))))
                        <div class="prose prose-invert prose-p:text-blue-100 prose-p:m-0 prose-p:leading-relaxed prose-strong:text-white prose-em:text-blue-200 max-w-none text-sm md:text-base [&_p]:inline [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4">{!! $item->hero_description !!}</div>
